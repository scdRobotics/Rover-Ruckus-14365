...
public class Lift extends OpMode {
   ...
   // plow.setPower(plowPower*.3);
    ...
    double extension = intakeExtension.getCurrentPosition();
    telemetry.addData("extension",extension);
    
    if (gamepad2.dpad_up==true && extension <= 9193) {
        intakeExtension.setPower(1);
    }
    else {
    if (gamepad2.dpad_down==true && extension >= 5970) {
        intakeExtension.setPower(-1);
        } 
        else {
            intakeExtension.setPower(0);
            //TODO:add some form of encoder lock - I couldn't figure it out
        }
    }
    //else {
    //    liftPin.setPosition(0.6); //deadbolt detach was 0.88
