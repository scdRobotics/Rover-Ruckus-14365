...
@TeleOp (name="Lift extension stop", group="Iterative Opmode")
public class Lift_extensionStop extends OpMode {
    ...
    telemetry.addData("Status", "Initialized");
        ...
        bucketRotation = hardwareMap.get(Servo.class,"bucketRotation");
    ...
    public void loop() {
        ...
        if (initialize){
            ...
            extensionPos = intakeExtension.getCurrentPosition();
            initialize = false;
        }
    ...
    //     liftPin.setPosition(0.4);
    //     telemetry.addData("pin","in");
    // } else {
    //     liftPin.setPosition(0.5);
    //     telemetry.addData("pin","out");
    // }
    ...
    bucketRotation.setPosition(bucketPos);
   ...
   // plow.setPower(plowPower*.3);
    ...
    telemetry.addData("extension",extensionPos);
    
    if (gamepad2.dpad_up==true && extensionPos <= 150) {
        extensionPos -= 150;
        intakeExtension.setTargetPosition(extensionPos);
        intakeExtension.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        
        intakeExtension.setPower(motorPower);
        //Wait for both motors to stop
        while ( intakeExtension.isBusy()){
    ...
    if (gamepad2.dpad_down==true && extensionPos >= 4000) {
        extensionPos += 150;
        intakeExtension.setTargetPosition(extensionPos);
        intakeExtension.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    telemetry.addData("lift",intakeLift.getCurrentPosition());
